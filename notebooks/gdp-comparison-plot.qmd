...
gdp_tab <-
  ...
  dplyr::filter(world_bank_14 < 10000 & world_bank_14 > 4900) |>
  dplyr::arrange(desc(world_bank_14))
...
gdp_tab <-
  ...
  dplyr::mutate(across(c(imf_1_13,
                         ...
                         united_nations_15),
                       ~ as.numeric(str_replace_all(.x, "[$,]", "")))) |>
  dplyr::filter(world_bank_14 < 10000 & world_bank_14 > 4000) |>
  dplyr::arrange(desc(world_bank_14))
...
p_Fiji <-
  ggplot2::ggplot(fiji) +
  ggplot2::geom_sf(fill = "#823549",
                   color = "#1D1E3C") +
  ggplot2::coord_sf(crs = 3460) +
  ggplot2::theme_void() +
  ggplot2::labs(subtitle = gdp_tab |> dplyr::filter(country_territory == "Fiji") |> dplyr::pull(country_territory),
       title = scales::dollar(gdp_tab |> dplyr::filter(country_territory == "Fiji") |>
                                   dplyr::pull(world_bank_14), big.mark = ".", scale = 1e-3, suffix = "B")) +
  ggplot2::theme(plot.subtitle = element_text(size = 20,
...
country_plots <- function(country, fill){
  country_data <-
    rnaturalearth::ne_countries(scale = 10, returnclass = "sf", country = country)

  p <- ggplot2::ggplot(country_data) +
    ggplot2::geom_sf(fill = fill,
            color = "#1D1E3C") +
    ggplot2::coord_sf() +
    ggplot2::theme_void() +
    ggplot2::labs(subtitle = gdp_tab |> dplyr::filter(country_territory == country) |> dplyr::pull(country_territory),
                  title = scales::dollar(gdp_tab |> dplyr::filter(country_territory == country) |> dplyr::pull(world_bank_14), big.mark = ".", scale = 1e-3, suffix = "B")) +
...
p_blank <-
  ggplot2::ggplot() +
  ggplot2::labs(subtitle = "Taylor Swift",
                title = "$6.30B") +
  ggplot2::theme_void() +
  ggplot2::theme(plot.subtitle = element_text(margin = margin(10, 0, 5.5, 0, unit = "pt"),
                                              size = 26,
                                              family = "lato",
                                              hjust = 0.5),
                 plot.title = element_text(hjust = 0.5,
                                           margin = margin(50, 0, 10, 0, unit = "pt")),
                 title = element_text(margin = margin(50, 0, 15, 0, unit = "pt"), 
                                      size = 28,
                                      family = "abril-fatface",
                                      hjust = 1,
                                      color = "#823549"))
...
gdp_plot <-
  cowplot::plot_grid(p_Kosovo,
                     ...
                     p_Eswatini,
                     align = "v") +
  cowplot::draw_image(here::here("images", "GDP-taylor.png"),
                      x = 0.4,
                      y = 0.32,
                      width = 0.2,
                      height = 0.2) +
  cowplot::draw_label("The GDP of Taylor",
                      x = 0.5,
                      y = 1.05,
                      hjust = 0.5,
                      vjust = 1,
                      ...
                      size = 60,
                      color = "#1D1E3C") +
  ggplot2::theme(plot.margin = margin(80, 0, 0, 0))
```
...
#| echo: true
#| eval: false
ggplot2::ggsave(filename = here::here("images", "gdp_plot.png"),
                plot = gdp_plot,
                device = png,
                path = NULL,
                width = 14,
