...
p_Fiji <-
  ...
  ggplot2::theme_void() +
  ggplot2::labs(title = gdp_tab |> dplyr::filter(country_territory == "Fiji") |> dplyr::pull(country_territory),
       subtitle = scales::dollar(gdp_tab |> dplyr::filter(country_territory == "Fiji") |>
                                   dplyr::pull(world_bank_14), big.mark = ".", scale = 1e-3, suffix = "B")) +
  ggplot2::theme(plot.subtitle = element_text(size = 12,
...
country_plots <- function(country, fill){
  ...
  p <- ggplot2::ggplot(country_data) +
    ...
    ggplot2::coord_sf() +
    ggplot2::theme_void() +
    ggplot2::labs(title = gdp_tab |> dplyr::filter(country_territory == country) |> dplyr::pull(country_territory),
                  subtitle = scales::dollar(gdp_tab |> dplyr::filter(country_territory == country) |> dplyr::pull(world_bank_14), big.mark = ".", scale = 1e-3, suffix = "B")) +
...
p_blank <-
  ggplot2::ggplot() +
  ggplot2::labs(title = "Taylor Swift",
                subtitle = "$6.30B") +
  ggplot2::theme_void() +
  ggplot2::theme(plot.subtitle = element_text(margin = margin(5.5, 0, 5.5, 0, unit = "pt"),
                                              size = 12,
                                              family = "lato",
                                              hjust = 0.5),
                 plot.title = element_text(hjust = 0.5,
...
gdp_plot <-
  cowplot::plot_grid(p_Kosovo,
                     ...
                     p_Eswatini,
                     align = "v") +
  cowplot::draw_image(here::here("notebooks", "t_swift.png"),
                      x = 0.4,
                      y = 0.28,
                      width = 0.2,
                      height = 0.2) +
  cowplot::draw_label("The GDP of Taylor",
                      ...
                      size = 32,
                      color = "#1D1E3C") +
  ggplot2::theme(plot.margin = margin(50, 0, 0, 0))
```
